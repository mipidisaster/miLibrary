...
void I2CDevice::PopGenClassParameters(void) {
/**************************************************************************************************
 * Populate generic parameters used within the I2CDevice class.
 *************************************************************************************************/
...
I2CDevice::I2CDevice(I2C_HandleTypeDef *I2C_Handle, GenBuffer<Form> *FormBuffer,
...
uint8_t I2CDevice::TransmitEmptyITChk(void) {
/**************************************************************************************************
...
uint8_t I2CDevice::TransmitComptITChk(void) {
/**************************************************************************************************
...
uint8_t I2CDevice::ReceiveToReadITChk(void) {
/**************************************************************************************************
...
uint8_t I2CDevice::BusNACKITChk(void) {
/**************************************************************************************************
...
uint8_t I2CDevice::BusSTOPITChk(void) {
/**************************************************************************************************
...
uint8_t I2CDevice::BusErrorITChk(void) {
/**************************************************************************************************
...
void I2CDevice::RequestTransfer(uint16_t devAddress, uint8_t size, CommMode mode, Request reqst) {
/**************************************************************************************************
...
I2CDevice::Form I2CDevice::GenericForm(uint16_t devAddress, uint8_t size, CommMode mode,
 ...
 *************************************************************************************************/
    I2CDevice::Form RequestForm = { 0 };        // Generate the "I2CDevice::Form" variable to
                                                // provide as output
...
void I2CDevice::FormW8bitArray(I2CDevice::Form *RequestForm, uint8_t *pData) {
/**************************************************************************************************
 * Link input 8bit array pointer to the provided I2C Request Form.
 *************************************************************************************************/
    RequestForm->ptr8bit        = pData;        // Pass data pointer to I2CForm
    RequestForm->UnionType      = Form::Array8bit;
        // Indicate that data type is 8bit array.
}

void I2CDevice::FormWGenBuffer(I2CDevice::Form *RequestForm, GenBuffer<uint8_t> *genBuff) {
/**************************************************************************************************
 * Link input GenBuffer pointer to the provided I2C Request Form.
 *************************************************************************************************/
    RequestForm->ptrGenBuf      = genBuff;      // Pass data pointer to I2CForm
    RequestForm->UnionType      = Form::GenBuffer8bit;
 ...
 *************************************************************************************************/
void I2CDevice::specificRequest(uint16_t devAddress, uint8_t size, uint8_t *pData,
 ...
 *************************************************************************************************/
    I2CDevice::Form RequestForm = this->GenericForm(devAddress, size, mode, reqst,
                                                    fltReturn, cmpFlag);
...
void I2CDevice::specificRequest(uint16_t devAddress, uint8_t size, GenBuffer<uint8_t> *genBuff,
                        CommMode mode, Request reqst, DevFlt *fltReturn, uint8_t *cmpFlag) {
/**************************************************************************************************
 * OVERLOADED function used to populate the internal I2C Form stack, with the input requested
 * communication.
...
uint8_t I2CDevice::GetFormWriteData(Form *RequestForm) {
/**************************************************************************************************
 ...
 *************************************************************************************************/
    ...
    if      (RequestForm->UnionType == Form::Array8bit) {   // If data type is 8bit array
        tempval = *(RequestForm->ptr8bit);                  // Retrieve data from array
        RequestForm->ptr8bit++;                             // Increment array pointer
    }
    else if (RequestForm->UnionType == Form::GenBuffer8bit){// If data is GenBuffer
        RequestForm->ptrGenBuf->OutputRead(&tempval);       // Retrieve data from buffer
    }

    if (this->curCount != 0) {
        this->curCount--;   // Decrease count (to keep track of data points communicated)
    }
    return (tempval);       // Return value outside of function
}

void I2CDevice::PutFormReadData(Form *RequestForm, uint8_t readdata) {
/**************************************************************************************************
 * Data read from the I2C external device is copied into the requested source location, as per
 * the I2C Request form
 *************************************************************************************************/
    if      (RequestForm->UnionType == Form::Array8bit) {   // If data type is 8bit array
        *(RequestForm->ptr8bit) = readdata;                 // Put data into array
        RequestForm->ptr8bit++;                             // Increment array pointer
    }
    else if (RequestForm->UnionType == Form::GenBuffer8bit){// If data is GenBuffer
        RequestForm->ptrGenBuf->InputWrite(readdata);       // Put data into buffer
    }
...
I2CDevice::DevFlt I2CDevice::poleMasterTransmit(uint16_t devAddress, uint8_t *pdata,
                                                uint8_t size) {
/**************************************************************************************************
...
I2CDevice::DevFlt I2CDevice::poleMasterReceive(uint16_t devAddress, uint8_t *pdata,
                                               uint8_t size) {
/**************************************************************************************************
 * Function will setup a communication link with the selected Device address. Where it will then
...
I2CDevice::DevFlt I2CDevice::poleDeviceRdy(uint16_t devAddress) {
/**************************************************************************************************
 * Function will setup a communication link with the selected Device address. Where it will then
 * read back the request amount of data from the device.
 * Any errors observed with the bus during the interaction will result in exiting of the function,
...
void I2CDevice::configTransmtIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::configTransCmIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::configReceiveIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::configBusNACKIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::configBusSTOPIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::configBusErroIT(InterState intr) {
/**************************************************************************************************
 * INTERRUPTS:
...
void I2CDevice::intMasterReq(uint16_t devAddress, uint8_t size, GenBuffer<uint8_t> *genBuff,
        CommMode mode, Request reqst, DevFlt *fltReturn, uint8_t *cmpFlag) {
/**************************************************************************************************
...
void I2CDevice::I2CInterruptStart(void) {
/**************************************************************************************************
...
void I2CDevice:: IRQEventHandle(void) {
/**************************************************************************************************
 * INTERRUPTS:
 ...
 *************************************************************************************************/
    ...
    if ( (TransmitEmptyChk() & TransmitEmptyITChk()) == 0x01) {     // If Transmit Buffer Empty
                                                                    // triggered
        this->DRWrite(GetFormWriteData( &(this->curForm) ));
    ...
    if ( (ReceiveToReadChk() & ReceiveToReadITChk()) == 0x01) {     // If Receive Buffer full
        ...
        // Put hardware read data into the source location
    }
    ...
    if ( (BusSTOPChk() & BusSTOPITChk()) == 0x01) {                 // If I2C Stop triggered
        ...
        *(this->curForm.Cmplt)  += (this->curForm.size - this->curCount);
        ...
        this->configReceiveIT(ITDisable);
    ...
    if ( (BusNACKChk() & BusNACKITChk()) == 0x01) {                 // If I2C NACK received
        ...
        *(this->curForm.Cmplt)  += (this->curForm.size - this->curCount);
            // Indicate how many data points have been transfered
...
void I2CDevice:: IRQErrorHandle(void) {
/**************************************************************************************************
 * INTERRUPTS:
