 ...
 *************************************************************************************************/
    if (this->Mux_HEnable != __null) {          // If a High Enable pin has been defined then
        this->Mux_HEnable->setValue(GPIO_HIGH); // Set the Pin HIGH to enable DeMux
    }

    if (this->Mux_LEnable != __null) {          // If a Low Enable pin has been defined then
        this->Mux_LEnable->setValue(GPIO_LOW);  // Set the Pin LOW to enable DeMux
    }
 ...
 *************************************************************************************************/
    if (this->Mux_HEnable != __null) {         // If a High Enable pin has been defined then
        this->Mux_HEnable->setValue(GPIO_LOW); // Set the Pin LOW to disable DeMux
    }

    if (this->Mux_LEnable != __null) {          // If a Low Enable pin has been defined then
        this->Mux_LEnable->setValue(GPIO_HIGH); // Set the Pin HIGH to disable DeMux
    }
...
_DeMuxFlt DeMux::updateselection(uint8_t newselection)
{
    ...
    for (temp = 0; temp != (this->inputsize); temp++) {     // Now use temporary variable to loop
        ...
        if (newselection & 1)                       // If lowest bit is "1", then
            this->Mux_A[temp].setValue(GPIO_HIGH);  // Set corresponding switch "HIGH"
        else                                        // If lowest bit is "0", then
            this->Mux_A[temp].setValue(GPIO_LOW);   // Set corresponding switch "LOW"
